...
//	@NotNull
	...
	public UserDto(@NotNull @NotBlank @Size(min = 6, max = 15) String loginId,
			...
			@NotBlank(message = "핸드폰 번호를 입력해주세요, 공백 문자는 포함하면 안됩니다.") @Size(min = 10, max = 30) @Pattern(regexp = "^[\\d]{11}+$", message = "'-' 기호 없이 전화번호를 입력해주세요.") String phoneNum,
			LocalDateTime createAt, Set<AuthorityDto> authorityDtoSet) {
		super();
	...
	public static UserDto from(User user) {
		...
		return new UserDto(user.getLoginId(), null, user.getName(), user.getNickname(), user.getBirthDate(),
				user.getGender(), user.getEmail(), user.getPhoneNum(), user.getCreateAt(), authorityDtoSet);
	}
